...
from common_function import matrix_distance_squared
...
def decompose(isometry, scheme='ccd'):
    ...
    lines = iso.shape[0]
    cols = iso.shape[1]
    log_rows = log2(lines) # n, 出
    log_cols = log2(cols) # m, 进

    _check_isometry(iso, log_rows, log_cols)
    ...
    if scheme == 'ccd':
        return _ccd(iso, log_rows, log_cols)

    return _knill(iso, log_rows, log_cols)
...
def _check_isometry(iso, log_rows, log_cols):
    if not log_rows.is_integer() or log_rows < 0:
        raise ValueError(
...
def _knill(iso, log_rows, log_cols):
    if log_rows < 2:
        raise ValueError(
            "Knill decomposition does not work on a 1 qubit isometry (N=2)."
        )

    unitary = _extend_to_unitary(iso, log_rows, log_cols)

    eigval, eigvec = np.linalg.eig(unitary)
    arg = np.angle(eigval)

    reg = QuantumRegister(log_rows)
    circuit = QuantumCircuit(reg)

    from qclib2.state_preparation.schmidt import initialize as schmidt # pylint: disable=import-outside-toplevel

    for i in range(2**log_rows):                            # The eigenvalues are not necessarily
                                                             # ordered.
        if np.abs(arg[i]) > 10**-7:
            ...
            circuit.x(list(range(log_rows)))
            circuit.mcp(arg[i], list(range(log_rows-1)), log_rows-1)
            circuit.x(list(range(log_rows)))
...
def _extend_to_unitary(iso, log_rows, log_cols):
    if log_rows == log_cols:      # The isometry v is already unitary.
        unitary = iso
        ...
        null_space = np.conj(scipy.linalg.null_space(iso.T)) # The complex conjugate of the null
                                                             ...
                                                             # U[:, M:] = W.T .
        unitary = np.zeros((2**log_rows, 2**log_rows), dtype=complex)
...
#   Column-by-column
def _ccd(iso, log_rows, log_cols):
    reg = QuantumRegister(log_rows)
    circuit = QuantumCircuit(reg)
    original_iso = np.array(iso)


    # 一行一行的变的
    for k in range(2**log_cols): # iteration through columns (starting with the second column k=1).
        g_k = _g_k(iso, log_rows, k)
        g_k.name = 'G'+str(k)

        circuit.append(g_k, reg)

        # print(np.round(iso, 2))
        # op = Operator(circuit).data
        # mat = np.round(np.dot(op,original_iso), 2)
        # print(mat)
    ...
    # op = Operator(circuit.inverse()) #.data
    # # mat = np.round(np.dot(op, iso), 2)
    # # print(mat)
    # print(matrix_distance_squared(op, original_iso))
    return circuit.inverse()

def _g_k(iso, log_rows, col_index):
    g_k = QuantumCircuit(log_rows)                   # Gate G columns index k, to be created.
    k_bin = '{:0{}b}'.format(col_index, log_rows)    # Binary representation of column index k.
    for i in range(log_rows):                        # G_k's subgate bit index i (s in the paper).
        target = log_rows - i - 1
        control = list(range(target))
        ancilla = list(range(target+1, log_rows))
        ...
        if _k_s(col_index, i) == 0 and _b(col_index, i+1) != 0:      # Condition defined in the
            ...
            mcg = _mc_gate(unitary, log_rows, control+ancilla, target, k_bin)
            mcg = mcg.reverse_bits()                                 # Qiskit little-endian.
            _update_isometry(iso, mcg)                               # Updates the isometry matrix.
            g_k.compose( mcg, list(range(log_rows)), inplace=True ) # Append "mcg" to the
                                                                     # operator "G_k".

        unitaries = _uc_unitaries(iso, log_rows, col_index, i)      # Generates single-qubit gates
                                                                     # matrices for the uniformly
                                                                     # controlled operation.
        ucg = _uc_gate(unitaries, log_rows, control, target)
        ucg = ucg.reverse_bits()                                     # Qiskit little-endian.
        _update_isometry(iso, ucg)                                   # Updates the isometry matrix.
        g_k.compose( ucg, list(range(log_rows)), inplace=True )     # Append "ucg" to the
                                                                     # operator "G_k".
    return g_k
...
def _update_isometry(iso, gate):
    gate_matrix = Operator(gate).data # "gate" matrix representation.
...
def _cnot_count_estimate(isometry, scheme='ccd'):
    ...
    log_rows = int(log2(iso.shape[0]))
    log_cols = int(log2(iso.shape[1]))

    if scheme == 'knill':
        return _cnot_count_estimate_knill(isometry, log_rows, log_cols)

    # CCD
    return _cnot_count_estimate_ccd(log_rows, log_cols)

def _cnot_count_estimate_knill(iso, log_rows, log_cols):
    """
    ...
    from qclib2.state_preparation.schmidt import cnot_count as schmidt_cnot_count # pylint: disable=import-outside-toplevel

    cnots = 0
    for i in range(2**log_rows):
        if np.abs(arg[i]) > 10**-15:
            ...
            if log_rows == 2:
                ...
                cnots += 16*log_rows**2 - 60*log_rows + 42

    return cnots

def _cnot_count_estimate_ccd(log_rows, log_cols):
    """
    Estimate the number of CNOTs to decompose the isometry using CCD.
    """
    cnots = 0
    for k in range(2**log_cols):
        k_bin = '{:0{}b}'.format(k, log_rows)
        # G_K

...
from unitary import cnot_count
...
def timer(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        return execution_time, r, cpu_time
    return wrapper


def cal_circuit_depth(qc: QuantumCircuit):
    """
    Calculate the depth of a QuantumCircuit Obj
    """
    return circuit_to_dagdependency(qc).depth()
        ...
        if not self.metrics:
            ...
            if qc:
                self.metrics = {
                    ...
                    'CNOT Count': cnot_count(qc),
                    'Depth': cal_circuit_depth(qc),
                    'Parallelism': cal_circuit_parallelism(qc),
                    'CPU time': cpu_time
                }
        return self.metrics
...
if __name__ == '__main__':
    ...
    print(f'Depth: {cal_circuit_depth(qc)}')

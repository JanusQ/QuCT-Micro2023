...
from downstream.synthesis.synthesis_model_tree import matrix_distance_squared

from scipy.stats import unitary_group

from downstream.synthesis.synthesis_model_tree import pkl_dump, pkl_load
from downstream.synthesis.synthesis_model_tree import SynthesisModel
...
# max_gate = 4**n_qubits
# min_gate = 5
# circuits = gen_random_circuits(min_gate = min_gate, max_gate = max_gate, gate_num_step = max([1, (max_gate+max_gate)//50]), n_circuits = 5, two_qubit_gate_probs=[3, 8], backend = backend, reverse = False, optimize = True, multi_process = True)
# upstream_model_path = f'./temp_data/upstream_model_q{n_qubits}_0220.pkl'
# upstream_model = RandomwalkModel(2, 20, backend = backend, travel_directions=('parallel', 'next'))
# upstream_model.train(circuits, multi_process = True)


model_name = f'q{n_qubits}_0220_REE_10000'

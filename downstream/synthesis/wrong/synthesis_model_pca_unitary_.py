...
def create_unitary_gate(connect_qubits):
    if len(connect_qubits) == 1:
        ...
        # params = np.concatenate([np.eye(2**n_connect_qubits).reshape((4**n_connect_qubits,)), np.zeros(4**n_connect_qubits)])
        params =np.eye(2**n_connect_qubits, dtype = np.complex128).reshape((4**n_connect_qubits,))
        return {
            'name': 'unitary',
            'qubits': list(connect_qubits),
            # 不知道有没有参数量更少的方法
            'params': params, #np.random.rand((4**n_connect_qubits)*2),
        }
...
def find_parmas(n_qubits, layer2gates, U, lr=1e-1, max_epoch=1000, allowed_dist=1e-2, n_iter_no_change=10, no_change_tolerance=1e-2, random_params=True, verbose = False):
    # np.random.RandomState()  # 看下keyzhe
    verbose = True
    param_size = 0
    params = []
    for layer in layer2gates:
    ...
    if random_params:
        params = np.random.normal(0, 1, (param_size,), dtype=np.complex128, requires_grad=True)
        # np.random.randn(num_layers, num_qubits, 3, requires_grad=True)
    else:
        params = np.array(params, dtype=np.complex128, requires_grad=True)
    ...
    @qml.qnode(dev)  # interface如果没有jax就不会有梯度
    def hilbert_test(params, U):
        ...
        qml.QubitUnitary(np.conj(U), wires=list(range(n_qubits)))
    ...
    # start_time = time.time()
    while True:
        (params, _,), loss_value = opt.step_and_cost(cost_hst, params, U)
        ...
        if verbose:
            ...
            assert matrix_distance_squared(np.conj(circuit_U.T) @ circuit_U, np.eye(2**n_qubits)) < 1e-5
            # assert np.allclose(np.conj(circuit_U.T) @ circuit_U, np.eye(2**n_qubits))
...
def _optimize(total_layers, new_layers, n_optimized_layers, U, lr, n_iter_no_change, n_qubits, allowed_dist):
    ...
    params, qml_dist = find_parmas(n_qubits, total_layers, U @ unchanged_part_matrix.T.conj(), max_epoch=1000, allowed_dist=allowed_dist,
                                    n_iter_no_change=n_iter_no_change, no_change_tolerance=allowed_dist/100, random_params=False, lr=lr, verbose = False)
    # allowed_dist/100
...
def synthesize(U, backend: Backend, allowed_dist=1e-10, heuristic_model: SynthesisModel = None, multi_process=True, verbose = False, lagre_block_penalty = 2):
    ...
    while now_dist > 1e-3:
        ...
        if not use_heuristic or (former_device_gate_vec is not None and jnp.allclose(device_gate_vec, former_device_gate_vec)):
            ...
            for n_subset_qubits in [2, 3] + list(range(max_n_subset_qubits, 3, -max_n_subset_qubits_step)): # TODO: 可能后面要设置成关于n_qubits的函数
    ...
    if now_dist > min_dist:
        params, qml_dist = find_parmas(n_qubits, total_layers, U, max_epoch=1000, allowed_dist=allowed_dist,
                                        n_iter_no_change=20, no_change_tolerance=0, random_params=False, verbose = True)
        # allowed_dist/100
        total_layers = assign_params(params, total_layers)

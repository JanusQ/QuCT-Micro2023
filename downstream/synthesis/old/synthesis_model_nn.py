    ...
    for layer_index, layer_gates in enumerate(layer2gates):
        ...
        for _ in range(20):
            ...
            device_gate_vecs.append(np.concatenate(
                [device_vec, gate_vec], axis=0, dtype=jnp.int16))
            depths.append(len(layer2gates)-layer_index)

    return device_gate_vecs, Us, depths
...
class SynthesisModel():
    ...
    def construct_data(self, circuits=None, multi_process=False):
        ...
        depths = np.array(depths, dtype=np.int32)#[:100000]

        print('len(Us, gate_vecs, depths) = ', len(Us))

        self.data = Us, device_gate_vecs, depths
    ...
    def construct_model(self):
        ...
        Us, gate_vecs, depths = self.data

        self.vec_model = KNeighborsClassifier(n_neighbors=1)
        self.vec_model.fit(Us, gate_vecs)
    ...
    def synthesize(self, U, allowed_dist=5e-2):
        ...
        n_qubits = self.n_qubits
        vec_model: KNeighborsClassifier = self.vec_model
        upstream_model = self.upstream_model
        ...
        while now_dist > allowed_dist:
            ...
            # start_time = time.time()
            device_gate_vec = vec_model.predict([transformU(inv_now_U)])[0]  # 花了半秒钟

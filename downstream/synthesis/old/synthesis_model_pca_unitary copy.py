...
class PCA():
    ...
    def transform(self, X) -> jnp.array:
        reduced_matrices: jnp.array = vmap(_transform, in_axes=(0, None, None))(X, self.X_mean, self.V_k)
    ...
    def synthesize(self, U, allowed_dist=5e-2, use_heuristic = True, multi_process = True):
        ...
        def create_unitary_gate(connect_qubits):
            if len(connect_qubits) == 1:
                ...
                return {
                    ...
                    'params': np.random.rand((4**n_connect_qubits)*2),  # 不知道有没有参数量更少的方法
                }
        ...
        while now_dist > allowed_dist:
            ...
            if use_heuristic and (former_device_gate_vec is None or jnp.allclose(device_gate_vec, former_device_gate_vec)):
                ...
                for layer_gates in layer2gates:
                    for gate in layer_gates:
                        ...
                        if len(set_indexs) == 0:
                            ...
                            connected_sets[_index] = list(set(connected_sets[_index]))
                ...
                connected_qubits = set(connected_qubits)
                
                single_qubits = [_qubit for _qubit in involved_qubits if _qubit not in connected_qubits]
                
                layer_gates = []
                for _qubit in single_qubits:
                    layer_gates.append({
                        'name' : 'u',
                        'qubits': [_qubit],
                        'params': np.random.rand(3) * 2 * np.pi,
                ...
                for _connected_set in connected_sets:
                    _connected_set.sort()
                    _n_involved_qubits = len(_connected_set)
                    layer_gates.append({
                        'name' : 'unitary',
                        'qubits': _connected_set,
                        'params': np.random.rand((4**_n_involved_qubits)*2),
                    })
                layer2gates = [layer_gates]
            else:
                '''explore最优的子电路'''
                ...
                for n_subset_qubits in (1, 2, 3):
                    for subset_qubits in combinations(list(range(n_qubits)), n_subset_qubits):
                        ...
                        layer_gates = [{
                            'name' : 'unitary',
                            'qubits': subset_qubits,
                            'params': np.random.rand((4**len(subset_qubits))*2),
                        }]
                        if multi_process:
                            futures.append((
                            ...
                            futures.append((subset_qubits, layer_gates, (params, dist)))
                ...
                for future in futures:
                    ...
                    if dist_decrement > 0:
                        dist_decrement /= 1.2**len(subset_qubits)  # 比特数多虽然能够减少的快，但是代价大
                    else:
                    ...
                    if dist_decrement > max_dist_decrement:
                        ...
                        layer2gates = assign_params(params, [layer_gates])
...
def synthesize_1q_2q(U, allowed_dist=5e-2):
    layer2gates = []
    ...
    if len(U) == 8:
        ...
        raise Exception('U with', U.shape, 'is not a matrix of a two-qubit circuit or a three-qubit circuit')

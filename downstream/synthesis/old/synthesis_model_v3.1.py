...
class SynthesisModel():
    def __init__(self, upstream_model: RandomwalkModel, name=None):
        ...
        # 放到外面看下会不会有jax.jit的加速
        '''
        TODO: 对比下hilbert_test和local hilbert_test，probs和expval的速度差异
        '''
        '''不放在这每次find_params都会有秒级别的初始化时间'''
        @qml.qnode(dev, interface="jax")
        def hilbert_test(params, layer2gates, U):
        ...
        self.hilbert_test = hilbert_test
        
        def cost_hst(params, layer2gates, U):
            probs = hilbert_test(params, layer2gates, U)
            # return (1 - probs)**2
            return (1 - probs[0])
        
        self.cost_hst = cost_hst
        return
    ...
    def construct_data(self, min_n_gate = None, max_n_gate = None, circuits=None, multi_process = False):
        ...
        if circuits is None:
            # max_n_gate =   # 4**n_qubits
            '''生成用于测试的模板电路'''
            circuits = gen_random_circuits(min_gate=min_n_gate, max_gate=max_n_gate, gate_num_step=max([max_n_gate//50, 1]), n_circuits=20,
    ...
    def find_parmas(self, layer2gates, U, lr=1e-1, max_epoch=1000, allowed_dist=1e-2, n_iter_no_change=10):
        ...
        while True:
            _time = time.time()
            loss_value, gradient = jax.value_and_grad(self.cost_hst)(params, layer2gates, U)  # 需要约1s
    ...
    def synthesize(self, U):
        ...
        # TODO: if nowU is identity, return 
        now_dist = 1
        ...
        while now_dist > 5e-2:
            ...
            params, dist =  self.find_parmas(total_layer2gates, U, max_epoch=50, allowed_dist=1e-2, n_iter_no_change = 5) # 有时候甚至会长，因为优化的问题

...
from sklearn.neural_network import MLPClassifier as DNN  # MLPClassifier as DNN
...
class SynthesisModel():
    ...
    def construct_model(self):
        ...
        self.model = DNN(
            (4**n_qubits, 2**n_qubits, 2**n_qubits, n_qubits), verbose=True, n_iter_no_change=10)
        self.model.fit(Us, gate_vecs)

        output = self.model.predict(Us[:10000])
...
@ray.remote
def synthesize(synthesis_model: SynthesisModel, U: jnp.array, max_n_iter=1000, allowed_dist=5e-2):
    return synthesize(synthesis_model, U, max_n_iter, allowed_dist)

def synthesize(synthesis_model: SynthesisModel, U: jnp.array, max_n_iter=1000, allowed_dist=5e-2):
    print('start synthesis')
    n_qubits = synthesis_model.n_qubits
    U_to_vec_model: DecisionTreeClassifier = synthesis_model.U_to_vec_model
    neural_model: DNN = synthesis_model.model
    ...
    while now_dist > allowed_dist:
        ...
        # start_time = time.time()
        qiskit_circuit = transpile(qiskit_circuit, coupling_map=synthesis_model.backend.coupling_map, optimization_level=3, basis_gates=[
            'u', 'cz'], initial_layout=[qubit for qubit in range(n_qubits)])
        # print(2, time.time() - start_time)

        # start_time = time.time()
        total_layer2gates = qiskit_to_layered_circuits(
            qiskit_circuit, divide=False, decoupling=False)['layer2gates']
        # print(3, time.time() - start_time)  # 0.0046303s

        # print(qiskit_circuit)
        # start_time = time.time()
        params, dist = synthesis_model.find_parmas(n_qubits, total_layer2gates, inv_U @ unchanged_part_matrix.T.conj(), max_epoch=50, allowed_dist=allowed_dist,
        ...
        iter_count += 1
        if max_n_iter <= iter_count:
            break

    '''TODO: 当前的电路要逆一下才是U的电路'''
    return total_layer2gates, now_dist

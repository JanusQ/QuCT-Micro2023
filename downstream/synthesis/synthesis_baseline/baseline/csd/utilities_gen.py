...
def complex2pol(c: np.complex_):
    rho = np.sqrt(c.real ** 2 + c.imag ** 2)
    phi = np.angle(c)
    return rho, phi


def pol2complex(rho: float, phi: float):
    return rho * (np.cos(phi) + np.sin(phi) * 1j)


def normalize_global_phase(mat1: np.ndarray, mat2: np.ndarray):
    # 2D ndarray
    assert len(mat1.shape) == 2 and len(mat2.shape) == 2 and mat1.shape == mat2.shape
    # dtype == np.complex
    assert mat1.dtype == np.complex_ and mat2.dtype == np.complex_

    m, n = mat1.shape
    i, j = 0, 0
    while i < m and j < n and (abs(mat1[i][j]) < 1e-6 or abs(mat2[i][j]) < 1e-6):
        if j < n:
            j += 1
        else:
            i += 1
    ...
    p1, p2 = np.angle(mat1[i][j]), np.angle(mat2[i][j])
    c1, c2 = pol2complex(1, p1), pol2complex(1, p2)

    print('delta global phase', p1, p2, p2 - p1)

    return mat1 / c1, mat2 / c2


def get_avg_phase(mat: np.ndarray):
    avg_p = 0
    for l in mat:
        for elm in l:
            _, p = complex2pol(elm)
...
def normalize_global_phase2(mat1: np.ndarray, mat2: np.ndarray):
    # 2D ndarray
    assert len(mat1.shape) == 2 and len(mat2.shape) == 2 and mat1.shape == mat2.shape
    # dtype == np.complex
    assert mat1.dtype == np.complex_ and mat2.dtype == np.complex_

    avg_p1, avg_p2 = get_avg_phase(mat1), get_avg_phase(mat2)
    c1, c2 = pol2complex(1, avg_p1), pol2complex(1, avg_p2)
    mat1 /= c1
    mat2 /= c2

...
def aposmm(H, persis_info, gen_specs, libE_info):
    ...
    while 1:
        ...
        for ind in starting_inds:
            if len([p for p in local_opters.values() if p.is_running]) < user_specs.get('max_active_runs', np.inf):
                ...
                x_new = local_opter.iterate(
                    local_H[ind][fields_to_pass])  # Assuming the second point can't be ruled optimal
        ...
        if num_samples_needed > 0:
            persis_info = add_k_sample_points_to_local_H(num_samples_needed, user_specs, persis_info, n, comm, local_H,
                                                         sim_id_to_child_inds)
            new_inds_to_send_mgr = new_inds_to_send_mgr + list(range(len(local_H) - num_samples_needed, len(local_H)))

        if not user_specs.get('standalone'):
            send_mgr_worker_msg(comm, local_H[new_inds_to_send_mgr + new_opt_inds_to_send_mgr][
                [i[0] for i in gen_specs['out']]])
        something_sent = True
...
def update_local_H_after_receiving(local_H, n, n_s, user_specs, Work, calc_in, fields_to_pass):
    for name in ['f', 'x_on_cube', 'grad', 'fvec', 'jac']:
        if name in fields_to_pass:
            assert name in calc_in.dtype.names, name + " must be returned to persistent_aposmm for localopt_method: " + \
...
def update_history_dist(H, n):
    ...
    for new_ind in new_inds:
        ...
        if p[new_ind]:
            ...
            H['dist_to_unit_bounds'][new_ind] = min(min(np.ones(n) - H['x_on_cube'][new_ind]),
...
def initialize_APOSMM(H, user_specs, libE_info):
    ...
    if 'components' in user_specs:
        ...
        if 'lm_jac_func' in user_specs['standalone']:
            local_H_fields += [('jac', float, (user_specs['components'], n))]
    ...
    if len(H):
        ...
        if user_specs['localopt_method'] in ['LD_MMA', 'blmvm']:
            assert 'grad' in H.dtype.names, "Must give 'grad' values to persistent_aposmm in gen_specs['in'] when using 'localopt_method'" + \
                                            user_specs['localopt_method']
        ...
        over_written_fields = ['dist_to_unit_bounds', 'dist_to_better_l', 'dist_to_better_s', 'ind_of_better_l',
                               'ind_of_better_s']
    ...
    assert n_s > 0 or user_specs[
        'initial_sample_size'] > 0, "APOSMM requires a positive initial_sample_size, or some existing points in order to determine where to start local optimization runs."
...
def initialize_children(user_specs):
    ...
    elif user_specs['localopt_method'] in ['scipy_least_squares_lm']:
        fields_to_pass = ['x_on_cube', 'fvec', 'jac']
    else:

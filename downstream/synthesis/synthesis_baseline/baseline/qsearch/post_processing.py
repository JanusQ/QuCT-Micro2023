    ...
    def post_process_circuit(self, result, options=None):
        ...
        for gate in single_qubit_names:
            ...
            while len(components) > 1:
                newstr = components[0] + identitystr + "".join([component + gate for component in components[1:-1]]) + \
                         components[-1]
                newcirc = eval(newstr)
        ...
        options.logger.logprint(
            "Post-processing removed {}, or {}% of the single qubit gates".format(initial_count - final_count, 100 * (
...
class LEAPReoptimizing_PostProcessor(Compiler, PostProcessor):
    ...
    def post_process_circuit(self, result, options=None):
        ...
        """
        if str(result['structure']).count('CNOT') <= (
        options.weight_limit if 'weight_limit' in options and options.weight_limit else options.reoptimize_size):
            return result
    ...
    def compile(self, options=Options()):
        ...
        logger = options.logger if "logger" in options else logging.Logger(verbosity=options.verbosity,
                                                                           stdout_enabled=options.stdout_enabled,
        ...
        if recovered_outer is None:
            ...
            if 'cut_depths' in options:
                ...
                midpoints = [1] + [pt + int((pt - prev) / 2) for pt, prev in
                                   zip(options.cut_depths[1:], options.cut_depths)]
        ...
        try:
            while True:
                ...
                for point in insertion_points:
                    ...
                    if options.gateset.d ** qudits != np.shape(U)[0]:
                        raise ValueError(
                            "The target matrix of size {} is not compatible with qudits of size {}.".format(
                                np.shape(U)[0], self.options.gateset.d))

                    I = gates.IdentityGate(d=options.gateset.d)

                    initial_layer = options.initial_layer if 'initial_layer' in options else options.gateset.initial_layer(
                        qudits)
                    search_layers = options.gateset.search_layers(qudits)

                    if len(search_layers) <= 0:
                        logger.logprint(
                            "This gateset has no branching factor so only an initial optimization will be run.")
                        root = initial_layer
                        result = options.solver.solve_for_unitary(options.backend.prepare_circuit(root, options),
                                                                  options)
                    ...
                    if recovered_state == None:
                        ...
                        #             0            1      2         3         4        5
                        child_checkpoint.save(
                            (queue, best_depth, best_value, best_pair, tiebreaker, timer() - startime))
                    else:
                        queue, best_depth, best_value, best_pair, tiebreaker, rectime = recovered_state
                        logger.logprint(
                            "Recovered state with best result {} at depth {}".format(best_value, best_depth))
                    ...
                    while len(queue) > 0:
                        ...
                        for _ in range(0, beams):
                            ...
                            popped.append(tup)
                            logger.logprint("Popped a node with score: {} at depth: {}".format((tup[2]), tup[1]),
                                            verbosity=2)

                        then = timer()
                        new_steps = [
                            (current_tup[5].inserting(search_layer[0], depth=point), current_tup[1], search_layer[1])
                            for search_layer in search_layers for current_tup in popped]
                        for step, result, current_depth, weight, time_cost in parallel.solve_circuits_parallel(
                                new_steps):
                            cpu_time += time_cost
                            current_value = options.objective.gen_eval_func(step, options)(result[1])
                            new_depth = current_depth + weight
                            if (current_value < best_value and (
                                    best_value >= options.threshold or new_depth <= best_depth)) or (
                                    current_value < options.threshold and new_depth < best_depth):
                            ...
                            if depth is None or new_depth < depth - 1:
                                ...
                                h(step, result[1], new_depth, options), new_depth, current_value, tiebreaker, result[1],
                                step))
                                tiebreaker += 1
                        logger.logprint("Layer completed after {} seconds".format(timer() - then), verbosity=2)
                        if (options.weight_limit is not None and best_depth >= options.weight_limit - 1) or (
                                'reoptimize_size' in options and best_depth >= options.reoptimize_size - 1):
                            break
                        child_checkpoint.save(
                            (queue, best_depth, best_value, best_pair, tiebreaker, rectime + (timer() - startime)))

                    logger.logprint(
                        "Finished compilation at depth {} with score {} after {} seconds.".format(best_depth,
                                                                                                  best_value,
                                                                                                  rectime + (
                                                                                                              timer() - startime)))
                    ...
                    if best_value < options.threshold and new_circuit_depth < best_circuit_depth:
                        logger.logprint(
                            f"With starting point {point} re-optimized from depth {best_circuit_depth} to depth {new_circuit_depth}")
                        ...
                        midpoints = [i - (best_circuit_depth - new_circuit_depth) for i in midpoints if
                                     (i - (point + window_size)) > 0]
                        logger.logprint(f'new midpoints: {midpoints}')
                        child_checkpoint.save(None)
                        child_checkpoint.save_parent(
                            (overall_best_pair, start_depth, midpoints, start_point, overall_best_value))
            ...
            parallel.done()
        logger.logprint(
            "Finished all compilations at depth {} with score {} after {} seconds.".format(best_circuit_depth,
                                                                                           overall_best_value,
                                                                                           rectime + (
                                                                                                       timer() - overall_startime)))
        return {'structure': overall_best_pair[0], 'parameters': overall_best_pair[1], 'cpu_time': cpu_time}

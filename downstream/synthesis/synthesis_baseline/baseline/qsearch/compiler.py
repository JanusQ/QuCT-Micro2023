...
class SearchCompiler(Compiler):
    ...
    Options:
        ...
        objective : An Objective used for scoring the quality of a parameterization for both synthesis and search.
    ...
    def compile(self, options=Options()):
        ...
        if recovered_state == None:
            ...
            if options.load_error:
                logger.logprint(
                    "Failed to recover state from checkpoint.  Resolve the issue or delete the checkpoint to finish the compilation.",
                    0)
        ...
        try:
            while len(queue) > 0:
                ...
                then = timer()
                new_steps = [(successor[0], current_tup[1], successor[1]) for current_tup in popped for successor in
                             options.gateset.successors(current_tup[5])]
                for step, result, current_weight, weight, time_cost in parallel.solve_circuits_parallel(new_steps):
                    cpu_time += time_cost
                    current_value = options.objective.gen_eval_func(step, options)(result[1])
                    new_weight = current_weight + weight
                    if (current_value < best_value and (
                            best_value >= options.threshold or new_weight <= best_weight)) or (
                            current_value < options.threshold and new_weight < best_weight):
                    ...
                    if weight_limit is None or new_weight < weight_limit:
                        ...
                        h(step, result[1], new_weight, options), new_weight, current_value, tiebreaker, result[1],
                        step))
                        tiebreaker += 1
                logger.logprint("Layer completed after {} seconds".format(timer() - then), verbosity=2)
                checkpoint.save(
                    (options, queue, best_weight, best_value, best_pair, tiebreaker, rectime + (timer() - starttime)))
        finally:
            parallel.done()

        logger.logprint(
            "Finished compilation at weight {} with score {} after {} seconds.".format(best_weight, best_value,
                                                                                       rectime + (timer() - starttime)))
        parallel.done()
        return {'structure': best_pair[0], 'parameters': best_pair[1], 'cpu_time': cpu_time}

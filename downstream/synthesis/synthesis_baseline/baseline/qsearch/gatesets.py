...
class Gateset():
    ...
    def __init__(self):
        ...
        raise NotImplementedError(
            "Gatesets must implemented their own initializers and must set self.d to reflect the size of the qudits implemented in the gateset")
...
class QubitCNOTLinear(Gateset):
    ...
    def successors(self, circ, qudits=None):
        ...
        return [(circ.appending(layer[0]), layer[1]) for layer in
                linear_topology(self.cnot, self.single_gate, qudits, self.d, single_alt=self.single_alt,
...
class QubitCNOTRing(Gateset):
    ...
    def search_layers(self, n):
        ...
        if n == 2:
            return gates
        finisher = (
        ProductGate(NonadjacentCNOTGate(n, n - 1, 0), KroneckerGate(self.single_gate, *[I] * (n - 2), self.single_alt)),
        1)
...
class QubitCZLinear(Gateset):
    ...
    def successors(self, circ, qudits=None):
        ...
        skip_index = find_last_3_cnots_linear(circ)
        return [(circ.appending(layer[0]), layer[1]) for layer in
                linear_topology(self.two_gate, self.single_gate, qudits, self.d, single_alt=self.single_alt,
                                skip_index=skip_index)]
...
class QubitCNOTAdjacencyList(Gateset):
    ...
    def search_layers(self, n):
        if n == 2:
            return [(ProductGate(CNOTGate(), KroneckerGate(self.single_gate, self.single_gate)),
                     1)]  # prevents the creation of an extra cnot placement in the 2 qubit case
        ...
        for pair in self.adjacency:
            ...
            single_gates = [self.single_gate if i == pair[0] else self.single_alt if i == pair[1] else self.I for i in
                            range(0, n)]
...
def linear_topology(double_gate, single_gate, n, d, identity_gate=None, single_alt=None, double_weight=1,
                    single_weight=0, skip_index=None):
    weight = double_weight + 2 * single_weight
    if not identity_gate:
        identity_gate = IdentityGate(d=d)
    if single_alt is None:
        return [(KroneckerGate(*[identity_gate] * i, ProductGate(double_gate, KroneckerGate(single_gate, single_gate)),
                               *[identity_gate] * (n - i - 2)), weight) for i in range(0, n - 1) if
                skip_index is None or i != skip_index]
    else:
        return [(KroneckerGate(*[identity_gate] * i, ProductGate(double_gate, KroneckerGate(single_alt, single_gate)),
                               *[identity_gate] * (n - i - 2)), weight) for i in range(0, n - 1) if
                skip_index is None or i != skip_index]

...
from . import utils, gatesets, solvers, backends, parallelizers, heuristics, logging, checkpoints, assemblers, \
    comparison, objectives, compiler
...
def default_logger(options):
    return logging.Logger(verbosity=options.verbosity, stdout_enabled=options.stdout_enabled,
...
def default_eval_func(options):
    ...
    raise AttributeError(
        "Could not find option 'eval_func'.  This option can be used in some cases for backwards compatability, but generally 'objective' should be used instead.")


def default_error_func(options):
    if isinstance(options.objective, objectives.MatrixDistanceObjective) or isinstance(options.objective,
                                                                                       objectives.BackwardsCompatibleObjective):
        return comparison.matrix_distance_squared
    raise AttributeError(
        "Could not find option 'error_func'.  This option can be used in some cases for backwards compatability, but generally 'objective' should be used instead.")
...
def default_error_residuals(options):
    ...
    raise AttributeError(
        "No 'error_func' was found.  This option can be used in some cases for backwards compatability, but generally 'objective' should be used instead.")
...
def default_error_jac(options):
    if isinstance(options.objective, objectives.BackwardsCompatibleObjective):
        ...
        return comparison.matrix_distance_squared_jac
    raise AttributeError(
        "Could not find option 'error_jac'.  This option can be used in some cases for backwards compatability, but generally 'objective' should be used instead.")
...
def default_error_residuals_jac(options):
    if isinstance(options.objective, objectives.BackwardsCompatibleObjective):
        ...
        return comparison.matrix_residuals_jac
    raise AttributeError(
        "Could not find option 'error_residuals_jac'.  This option can be used in some cases for backwards compatability, but generally 'objective' should be used instead.")


def default_objective(options):
    if options.manually_entered("eval_func", "error_func", "error_jac", "error_residuals", "error_residuals_jac",
                                operator="any"):

...
def matrix_residuals_slice_jac(slices, A, B, J):
    JU = np.array(
        [np.append(np.reshape(np.real(K[slices]), (1, -1)), np.reshape(np.imag(K[slices]), (1, -1))) for K in J])
    return JU.T
...
def matrix_residuals_blacklist_jac(badrows, badcols, A, B, J):
    JU = np.array([np.append(np.reshape(np.real(np.delete(np.delete(K, badrows, 0), badcols, 1)), (1, -1)),
                             np.reshape(np.imag(np.delete(np.delete(K, badrows, 0), badcols, 1)), (1, -1))) for K in J])
    return JU.T

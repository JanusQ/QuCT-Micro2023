    ...
    def compile(self, options=Options()):
        ...
        while True:
            if 'timeout' in options and timer() - starttime > options.timeout:
                break
            opts = options.updated(initial_layer=initial_layer, local_threshold=options.delta * best_value,
                                   overall_starttime=starttime, overall_best_value=best_value,
                                   checkpoint=child_checkpoint)
            best_pair, best_value, best_depth, time_cost = sc.compile(opts)
            cpu_time += time_cost
        ...
        logger.logprint(
            "Finished all sub-compilations at depth {} with score {} after {} seconds.".format(total_depth, best_value,
                                                                                               (timer() - starttime)))
        return {'structure': best_pair[0], 'parameters': best_pair[1], 'cut_depths': depths, 'cpu_time': cpu_time}
...
class SubCompiler(Compiler):
    ...
    def compile(self, options=Options()):
        ...
        if options.gateset.d ** qudits != np.shape(U)[0]:
            raise ValueError(
                "The target matrix of size {} is not compatible with qudits of size {}.".format(np.shape(U)[0],
                                                                                                self.options.gateset.d))
        ...
        if len(search_layers) <= 0:
            ...
            return ((root, result[1]), value, cpu_time)
        ...
        if recovered_state == None:
            ...
            if depth == 0:
                return (best_pair, best_value, 0, cpu_time)
        ...
        try:
            while len(queue) > 0:
                ...
                then = timer()
                new_steps = [(current_tup[5].appending(search_layer[0]), current_tup[1], search_layer[1]) for
                             search_layer in search_layers for current_tup in popped]
                    ...
                    if (current_value < best_value and (
                        ...
                        if len(previous_bests_values) > 1:
                            with np.errstate(invalid='ignore', divide='ignore'):
                                try:
                                    slope, intercept, _rval, _pval, _stderr = linregress(previous_bests_depths,
                                                                                         previous_bests_values)
                                except:
                                    continue
                            predicted_best = slope * new_depth + intercept
                            delta = predicted_best - best_value
                            logger.logprint(f"Predicted best value {predicted_best} for new best with delta {delta}",
                                            verbosity=2)
                            if not np.isnan(
                                    predicted_best) and best_value < options.overall_best_value and delta < 0 and (
                                    'min_depth' not in options or new_depth >= options.min_depth):
                                parallel.done()
                                return (best_pair, best_value, best_depth, cpu_time)
                        previous_bests_depths.append(best_depth)
                        previous_bests_values.append(best_value)

                    if depth is None or new_depth < depth:
                        heapq.heappush(queue, (
                        h(step, result[1], new_depth, options), new_depth, current_value, tiebreaker, result[1], step))
                        tiebreaker += 1
        ...
        logger.logprint(
            "Finished compilation at depth {} with score {} after {} seconds.".format(best_depth, best_value,
                                                                                      rectime + (timer() - starttime)))
        return (best_pair, best_value, best_depth, cpu_time)

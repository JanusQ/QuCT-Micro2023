...
def error_stateprep_distsq(circuit, parameters, target, options):
    return 1 - np.real(
        np.vdot(np.dot(target, options.initial_state), np.dot(circuit.matrix(parameters), options.initial_state)))
...
def residuals_blacklist(circuit, parameters, target, options):
    return comparison.matrix_residuals_blacklist(options.badrows, options.badcols, target, circuit.matrix(parameters),
                                                 options.fullsize_)


def residuals_blacklist_jac(circuit, parameters, target, options):
    return comparison.matrix_residuals_blacklist_jac(options.badrows, options.badcols, target,
                                                     circuit.matrix(parameters), jacs)
...
def cost_combo_linear(circuit, parameters, target, options):
    return options.cost1(circuit, parameters, target, options) + options.cost_coefficient * options.cost2(circuit,
                                                                                                          parameters,
                                                                                                          target,
                                                                                                          options)
...
def cost_combo_linear_jac(circuit, parameters, target, jacs, options):
    return options.cost1_jac(circuit, parameters, target, jacs, options) + options.cost_coefficient * options.cost2_jac(

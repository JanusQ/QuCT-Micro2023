...
try:
    from qsrs import LeastSquares_Jac_SolverNative, BFGS_Jac_SolverNative, native_from_object, matrix_residuals, \
...
def default_solver(options, x0=None):
    ...
    if qudits < 1:
        logger.logprint(
            "Smart default fell back to COBYLA_Solver.  Pass a different Solver to SearchCompiler for better results.",
            verbosity=1)
...
class CMA_Solver(Solver):
    ...
    def solve_for_unitary(self, circuit, options, x0=None):
        ...
        xopt, _ = cma.fmin2(error_func, initial_guess, 0.25, {'verb_disp': 0, 'verb_log': 0, 'bounds': [0, 2 * np.pi]},
                            restarts=2)
...
class COBYLA_Solver(Solver):
    ...
    def solve_for_unitary(self, circuit, options, x0=None):
        ...
        x = sp.optimize.fmin_cobyla(error_func, initial_guess, cons=[lambda x: np.all(np.less_equal(x, 2 * np.pi))],
                                    rhobeg=0.5, rhoend=1e-12, maxfun=1000 * circuit.num_inputs)
...
class BFGS_Jac_Solver(Solver):
    ...
    def solve_for_unitary(self, circuit, options, x0=None):
        ...
        result = sp.optimize.minimize(error_jac, np.random.rand(circuit.num_inputs) * 2 * np.pi if x0 is None else x0,
                                      method='BFGS', jac=True, tol=options.threshold)
        xopt = result.x
        return (circuit.matrix(xopt), xopt)
...
class LeastSquares_Jac_Solver(Solver):
    ...
    def solve_for_unitary(self, circuit, options, x0=None):
        ...
        if options.max_quality_optimization:
            result = sp.optimize.least_squares(error_residuals,
                                               np.random.rand(circuit.num_inputs) * 2 * np.pi if x0 is None else x0,
                                               error_residuals_jac, method="lm", ftol=5e-16, xtol=5e-16, gtol=1e-15)
        else:
            result = sp.optimize.least_squares(error_residuals,
                                               np.random.rand(circuit.num_inputs) * 2 * np.pi if x0 is None else x0,
                                               error_residuals_jac, method="lm")
        xopt = result.x
        return (circuit.matrix(xopt), xopt)

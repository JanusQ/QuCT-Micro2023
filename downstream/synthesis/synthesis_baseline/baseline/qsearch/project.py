...
class Project:
        ...
        if self.rank == 0:
            ...
            try:
                ...
                with open(self.projfile, "rb") as projfile:
                    ...
                    self.logger = logging.Logger(self.options.stdout_enabled,
                                                 os.path.join(path, "{}-project-log.txt".format(self.name)),
                ...
                self.logger = logging.Logger(True, os.path.join(path, "{}-project-log.txt".format(self.name)),
                                             verbosity=1)
    ...
    def add_compilation(self, name, U, options=None, handle_existing=None, **extraargs):
        ...
        if name in self._compilations:
            ...
            if handle_existing == "ignore" or np.array_equal(U, self._compilations[name][
                "options"].target):  # handle the case where this is an attempt to re-enter the same compilation
                if options is None and len(extraargs) == 0:
                    ...
                    warn("Compilation {} already has made progress.  To change it, reset it first..".format(name),
                ...
                warn(
                    "A compilation with name {} already exists.  To change it, remove it and then add it again.".format(
                        name), RuntimeWarning, stacklevel=2)
                return
        ...
        if not keyword in ["verbosity", "stdout_enabled"]:  # "safe" keywords here
            for name in self._compilations:
                ...
                if s == Project_Status.COMPLETE or s == Project_Status.PROGRESS:
                    warn(
                        "This project contains compilations which have been completed or have been started.  Please call reset() to clear this progress before changing configurations.",
                        RuntimeWarning, stacklevel=2)
                    return
        self.options.update(**{keyword: value})
        self._save()
    ...
    def configure_compiler_override(self, keyword, value):
        ...
        warn(
            "Using this method could result in crashes, infinite loops, or other undefined behavior.  It is safer to reset the project and configure using project[\"keyword\"]=value.  Only use this method if the risk of error is worse than losing intermediate progress.")
        self.options.update(**{keyword: value})
        self._save()
    ...
    def remove_compilation(self, name):
        ...
        """
        warn(
            "remove_compilation(name) is deprecated and will be removed in a future release.  Use clear(name) instead.",
    ...
    def run(self):
        ...
        for name in self._compilations:
            ...
            if self._compilation_status(name) == Project_Status.COMPLETE:
                continue
            sublogger = logging.Logger(runopt.stdout_enabled, os.path.join(self.folder, "{}-log.txt".format(name)),
                                       runopt.verbosity)

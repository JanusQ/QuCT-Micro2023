...
class SynthesisModel():
    ...
    def construct_data(self, circuits, multi_process=False, random_parm_per_circuit = 10):
        ...
        def gen_data(circuit_info, n_qubits, index):
            ...
            for layer_index, layer_gates in enumerate(layer2gates):
                ...
                if len(layer_gates) == 0:
                    continue
                for _ in range(random_parm_per_circuit):
...
class SynthesisModelNN(SynthesisModel):
    ...
    def construct_model(self, data):
        ...
        self.pca_model = PCA(Us, reduced_prop=.8, max_k=15)
        
        Us = self.pca_model.transform(Us)
        Us = [flatten(U) for U in Us]
        
        # metric= self.flatten_matrix_distance_squared, 
        self.nbrs = NearestNeighbors(n_neighbors=10, n_jobs = -1).fit(Us)  # algorithm='ball_tree', 
        self.Vs = Vs
...
def find_parmas(n_qubits, layer2gates, U, lr=1e-1, max_epoch=1000, allowed_dist=1e-2, n_iter_no_change=10, no_change_tolerance=1e-2, random_params=True, verbose=False, min_loss = None):
    ...
    # if min_loss is None: # 有些是负的就让他早点结束吧
    #     min_loss = 1e2
    # min_loss *= 1.2
    min_loss = 1e2
    ...
    while True:
        ...
        if verbose:
            ...
            print('Epoch: {:5d} | Dist: {:.5f}'.format(
                epoch, loss_value))
            
        # circuit_U: jnp.array = layer_circuit_to_matrix(
        #                 layer2gates, n_qubits, params)
        # matrix_dist = matrix_distance_squared(circuit_U, U)
        # print( 1- math.sqrt(1-loss_value) , matrix_dist )
        ...
        # if epoch < max([n_iter_no_change, 20]):
...
# @ray.remote
# def find_parmas_remote(n_qubits, layer2gates, U, max_epoch, allowed_dist, n_iter_no_change, no_change_tolerance, random_params, min_loss):
#     return find_parmas(n_qubits, layer2gates, U, max_epoch=max_epoch, allowed_dist=allowed_dist,
#                        n_iter_no_change=n_iter_no_change, no_change_tolerance=no_change_tolerance, random_params=random_params, min_loss = min_loss)[:2]


def _optimize(total_layers, new_layers, n_optimized_layers, U, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist, remote = True):
    if not remote:
        total_layers = copy.deepcopy(total_layers)
    ...
    params, now_dist, epoch_spent = find_parmas(n_qubits, total_layers, U @ unchanged_part_matrix.T.conj(), max_epoch=100, allowed_dist=allowed_dist,
                                                n_iter_no_change=n_iter_no_change, no_change_tolerance=now_dist/100, random_params=False, lr=lr, verbose=False, min_loss = now_dist)
    # allowed_dist/100
    ...
    return remained_U, total_layers, now_dist, epoch_spent, time.time() - start_time


@ray.remote
def _optimize_remote(total_layers, new_layers, n_optimized_layers, invU, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist):
    return _optimize(total_layers, new_layers, n_optimized_layers, invU, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist, remote = False)


def synthesize(U, backend: Backend, allowed_dist=1e-5, heuristic_model: SynthesisModel = None, multi_process=True, 
               verbose=False, lagre_block_penalty=2, heuristic_ratio = .5, timeout = 5*3600, synthesis_log = None):
    ...
    while now_dist > allowed_dist:
        ...
        # if not use_heuristic or device_gate_vec is None or (former_device_gate_vec is not None and jnp.allclose(device_gate_vec, former_device_gate_vec)):
            ...
            # 增加一个多层的，尽量减少冗余的
            for _layers in itertools.product(*([unitary_candidates]*2)):
                is_repeat = False
                ...
                for _candidate in _layers:
                    _new_candiate += _candidate
                unitary_candidates.append(_new_candiate)
        ...
        for candidate_layer in canditate_layers:
            ...
            if (iter_count+1) % 10 == 0 or now_dist < 1e-1:  # TODO: 需要尝试的超参数
        ...
        def cal_penalty(candidate_layer):
            penalty = 0
            for layer in candidate_layer:
                for gate in layer:
                    if gate['name'] != 'unitary':
                        penalty += len(gate['qubits'])
                    else:
                        penalty += lagre_block_penalty**(len(gate['qubits'])-1)
            return penalty
                        

        heuristic_takeeffact = False
        for future in futures:
            candidate_layer, (c_remained_U, c_total_layers, c_now_dist, c_epoch, subp_time) = future
        ...
        if max_dist_decrement == 0:  # 所有的candidate都在增长
            if verbose:
                print('Warning: no improvement in the exploration')
            future = random_choice(futures)
            candidate_layer, (remained_U, total_layers, now_dist, epoch_spent, subp_time) = future

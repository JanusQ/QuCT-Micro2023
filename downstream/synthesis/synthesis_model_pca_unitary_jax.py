...
class SynthesisModelNN(SynthesisModel):
    ...
    def construct_model(self, data):
        ...
        self.nbrs = NearestNeighbors(n_neighbors=6, n_jobs = -1).fit(Us)  # algorithm='ball_tree', 
        self.Vs = Vs
...
def _optimize(total_layers, new_layers, n_optimized_layers, U, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist, remote = True):
    ...
    params, qml_dist, epoch_spent = find_parmas(n_qubits, total_layers, U @ unchanged_part_matrix.T.conj(), max_epoch=100, allowed_dist=allowed_dist,
                                                n_iter_no_change=n_iter_no_change, no_change_tolerance=now_dist/100, random_params=False, lr=lr, verbose=False)
    # allowed_dist/100

...
def get_data(id, qiskit_circuit, coupling_map, mirror, trans=True):
    ...
    if trans == True:
        qiskit_circuit = transpile(new_qiskit_circuit, basis_gates=default_basis_gates, coupling_map=coupling_map,
...
def get_dataset_bug_detection(min_qubit_num, max_qubit_num, coupling_map, mirror):
    ...
    for n_qubits in range(min_qubit_num, max_qubit_num):
        ...
        dataset.append(
            get_data(f'bernstein_vazirani', al.bernstein_vazirani(get_bitstr(n_qubits - 1)), coupling_map, mirror))
        # algorithm.append(get_data(f'w_state', w_state.get_cir(n_qubits)))
        dataset.append(
            get_data(f'deutsch_jozsa', deutsch_jozsa.get_cir(n_qubits - 1, get_bitstr(n_qubits - 1)), coupling_map,
                     mirror))
        if n_qubits % 5 == 0:

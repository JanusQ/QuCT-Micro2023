...
for step  in range(1,2):    
    ...
    for n_qubits in range(5, 128, 1):
        ...
        dataset = gen_random_circuits(min_gate=n_qubits * 10, max_gate=n_qubits *20, n_circuits=1000 // n_qubits , two_qubit_gate_probs=[
                                  2, 3], gate_num_step=10, backend=backend, multi_process=True)

        upstream_model = RandomwalkModel(
            step, 20, backend=backend, travel_directions=('parallel', 'former'))
